...
echo "<body onload='start(\"$bildist\", 2000, 0)' class='alarm'>\n";
...
echo "<table class='table-alarm' cellspacing='0'>\n<tr>\n";

if ($show_fms_logo=='true')
{
echo "<td class='td-logo-alarm'><div class='logo-alarm'>FMS:</div></td>\n";
}
...
for ($i=1; $i<$fzg_zahl+1; $i++)
	...
	echo "<td class='fahrzeug-alarm'>".$fzg_status[$i]['fzgname']."&nbsp;&nbsp;</td>\n";

	switch ($fzg_status[$i]['fzgstatus'])
		{
		case "1":
		echo "<td class='fstatus-alarm eins'>".$fzg_status[$i]['fzgstatus']."</td>\n";
		break;
	
		case "2":
		echo "<td class='fstatus-alarm zwei'>".$fzg_status[$i]['fzgstatus']."</td>\n";
		break;

		case "3":
		echo "<td class='fstatus-alarm drei'>".$fzg_status[$i]['fzgstatus']."</td>\n";
		break;
	
		case "4":
		echo "<td class='fstatus vier'>".$fzg_status[$i]['fzgstatus']."</td>\n";
		break;

		case "5":
		echo "<td class='fstatus-alarm fuenf'>".$fzg_status[$i]['fzgstatus']."</td>\n";
		break;
	
		case "6":
		echo "<td class='fstatus-alarm sechs'>".$fzg_status[$i]['fzgstatus']."</td>\n";
		break;

		case "9":
		echo "<td class='fstatus-alarm neun'>".$fzg_status[$i]['fzgstatus']."</td>\n";
		break;
	
		default:
		echo "<td class='fstatus-alarm default'>".$fzg_status[$i]['fzgstatus']."</td>\n";
		break;
		}

	}

/* Datumsfunktion auskommentiert, hier ist der Unterschied zur Anzeige bei der Uhr
...
for ($i=1; $i<$fzg_zahl+1; $i++)
	...
	if ($status_ils == 'true')
		{
		echo "<td class='datum-alarm'>".date("d.m.Y / H:i:s",$fzg_status[$i]['datum'])." (".$fzg_status[$i]['aenderungdurch'].")</td>\n";
		
		switch ($fzg_status[$i]['lststatus'])
			{
			case 'C':
			echo "<td class='lstatus-alarm C'>".$fzg_status[$i]['lststatus']."</td>\n";
			break;

			default:
			echo "<td class='lstatus-alarm default'>".$fzg_status[$i]['lststatus']."</td>\n";
			break;			
			}

		} else {
		echo "<td colspan='2' class='lstatus-alarm'>".date("d.m.Y / H:i:s",$fzg_status[$i]['datum'])."</td>\n";
		}
